        ...
        <RadioGroup value={color} onChange={setColor}>
          ...
          <div className="flex justify-between space-x-8 mt-2">
            {colors.map((c) => {
              return (
                <RadioGroup.Option
                  className="ui-checked:text-onPrimaryBg ui-checked:bg-primaryBg ring-4 ui-checked:ring-primary ui-not-checked:ring-onNeutralBg h-20 w-full flex justify-center items-center font-bold uppercase cursor-pointer select-none"
                  value={c}
                  key={c}
                >
                  {c}
